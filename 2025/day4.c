...
void check_available_rolls(const Size s, char (*arr)[s.h]) {
    for (int i = 0; i < s.h; i++) {
        for (int j = 0; j < s.w; j++) {
            if (arr[i][j] == '.') {continue;}
            int count = 0;
            ...
            if (i > 0) { //check up and left|right
                if (arr[i-1][j] == '@') {++count;}
                if (j > 0) {if(arr[i-1][j-1] == '@') {++count;}}
                if (j < s.w-1) {if(arr[i-1][j+1] == '@') {++count;}}
            };
...
bool set_arr_x(const Size s, char (*arr)[s.h]) {
    for (int i = 0; i < s.h; i++) {
        for (int j = 0; j < s.w; j++) {
            if (arr[i][j] == '.') {continue;}
            int count = 0;
...
int main() {
    ...
    check_available_rolls(input_size, arr);
    set_arr_x(input_size, arr);
